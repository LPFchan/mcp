...
export function createTools(vaultPath) {
  ...
  function runGit(args, timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      const child = spawn('git', ['-C', vaultPath, ...args], {
      ...
      child.on('exit', (code, signal) => {
        ...
        if (code === 0) {
          ...
          reject(new Error(`git ${args[0]} exited ${code}${signal ? ' (signal ' + signal + ')' : ''}: ${stderr.trim()}`));
        }
      });
    });
  }
  ...
  async function gitRepoCheck() {
    if (hasGitRepo !== null) return hasGitRepo;
    try {
      const stat = await fs.stat(path.join(vaultPath, '.git'));
      hasGitRepo = stat.isDirectory();
    } catch {
      hasGitRepo = false;
    }
    return hasGitRepo;
    ...
    return new Promise((resolve) => {
      ...
      debounceTimer = setTimeout(async () => {
        ...
        const label = messages.length === 1
          ? messages[0]
          : `batch: ${messages.length} changes\n\n${messages.map((m) => `- ${m}`).join('\n')}`;

        try {
          const tmp = path.join(vaultPath, '.git-commit-msg');
          await fs.writeFile(tmp, label, 'utf8');
          await runGit(['add', '-A']);
          await runGit(['commit', '-F', tmp, '--allow-empty']);
          await fs.rm(tmp);
        } catch (err) {
          console.error('gitCommit failed:', err.message || err);
        } finally {
          for (const r of resolvers) r();
        }
      }, 2000);
  ...
  const TOOLS = {
    ...
    'create-note': {
      ...
      async run({ filename, folder, content }) {
        ...
        await scheduleCommit(`create-note: ${filename}`);
    ...
    'edit-note': {
      ...
      async run({ filename, folder, operation, content, oldString, newString }) {
        ...
        await scheduleCommit(`edit-note: ${filename} (${operation})`);
          const past = { append: 'appended', prepend: 'prepended', overwrite: 'overwritten', replace: 'replaced' }[operation];
          return `Note ${past} successfully\n\nModified file: ${path.relative(vaultPath, fp)}`;
      }
    },
    ...
    'move-note': {
      ...
      async run({ filename, folder, newFilename, newFolder }) {
        ...
        await scheduleCommit(`move-note: ${filename} → ${newFilename ?? filename}`);
    ...
    'create-directory': {
      ...
      async run({ path: dirPath }) {
        ...
        await scheduleCommit(`create-directory: ${dirPath}`);
    ...
    'delete-note': {
      ...
      async run({ filename, folder }) {
        ...
        await scheduleCommit(`delete-note: ${filename}`);
    ...
    'add-tags': {
      ...
      async run({ filename, folder, tags }) {
        ...
        await scheduleCommit(`add-tags: ${filename} (+${tags.length})`);
    ...
    'remove-tags': {
      ...
      async run({ filename, folder, tags }) {
        ...
        await writeFile(fp, rebuildNote(fm, body, updated));
        await scheduleCommit(`remove-tags: ${filename} (-${tags.length})`);
    ...
    'rename-tag': {
      ...
      async run({ oldTag, newTag }) {
        ...
        await scheduleCommit(`rename-tag: ${oldTag} → ${newTag} (${count} notes)`);
    ...
    'delete-directory': {
      ...
      async run({ path: dirPath }) {
        ...
        await scheduleCommit(`delete-directory: ${dirPath}`);
    ...
    'recover-from-trash': {
      ...
      async run({ filename, targetFolder }) {
        ...
        await fs.mkdir(path.dirname(dest), { recursive: true });
        await fs.rename(src, dest);
        await scheduleCommit(`recover-from-trash: ${filename}`);
