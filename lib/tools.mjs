...
export function createTools(vaultPath) {
  ...
  async function gitCommit(msg) {
    try {
      ...
      await execAsync(`git -C "${vaultPath}" commit -F "${tmp}" --allow-empty`);
      await fs.rm(tmp);
    } catch {}
  }
  ...
  const TOOLS = {
    ...
    'create-note': {
      ...
      async run({ filename, folder, content }) {
        ...
        await gitCommit(`create-note: ${filename}`);
    ...
    'edit-note': {
      ...
      async run({ filename, folder, operation, content, oldString, newString }) {
        ...
        await gitCommit(`edit-note: ${filename} (${operation})`);
    ...
    'move-note': {
      ...
      async run({ filename, folder, newFilename, newFolder }) {
        ...
        await gitCommit(`move-note: ${filename} → ${newFilename ?? filename}`);
    ...
    'create-directory': {
      ...
      async run({ path: dirPath }) {
        ...
        await gitCommit(`create-directory: ${dirPath}`);
    ...
    'delete-note': {
      ...
      async run({ filename, folder }) {
        ...
        await gitCommit(`delete-note: ${filename}`);
    ...
    'add-tags': {
      ...
      async run({ filename, folder, tags }) {
        ...
        await gitCommit(`add-tags: ${filename} (+${tags.length})`);
    ...
    'remove-tags': {
      ...
      async run({ filename, folder, tags }) {
        ...
        await writeFile(fp, rebuildNote(fm, body, updated));
        await gitCommit(`remove-tags: ${filename} (-${tags.length})`);
    ...
    'rename-tag': {
      ...
      async run({ oldTag, newTag }) {
        ...
        await gitCommit(`rename-tag: ${oldTag} → ${newTag} (${count} notes)`);
    ...
    'delete-directory': {
      ...
      async run({ path: dirPath }) {
        ...
        await gitCommit(`delete-directory: ${dirPath}`);
    ...
    'recover-from-trash': {
      ...
      async run({ filename, targetFolder }) {
        ...
        await fs.mkdir(path.dirname(dest), { recursive: true });
        await fs.rename(src, dest);
        await gitCommit(`recover-from-trash: ${filename}`);
