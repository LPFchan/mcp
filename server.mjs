...
if (!VAULT_PATH) { console.error('Usage: server.mjs <vault-path>'); process.exit(1); }

const TOKEN = process.env.MCP_TOKEN || 'f3044131e50d4a36e42d0db35edd06d515d9d62d322e0a0c36003ea2b0d05fcb';
const PORT = parseInt(process.env.PORT || '3000', 10);
const ALLOWED_ORIGIN = process.env.ALLOWED_ORIGIN || 'https://chat.lost.plus';

const VAULT_NAME = path.basename(VAULT_PATH).toLowerCase().replace(/\s+/g, '-');
const TRASH_PATH = path.join(VAULT_PATH, '.trash');

// ── Helpers ──────────────────────────────────────────────────────────────────

function resolvePath(folder, filename) {
  return path.join(folder ? path.join(VAULT_PATH, folder) : VAULT_PATH, filename);
}
...
function buildFrontmatter(fm, tags) {
...
function rebuildNote(fm, body, tags) {
...
function fuzzyContentMatch(query, content) {
...
async function search(query, searchType, caseSensitive, subPath, fuzzy = false) {
  const root = subPath ? path.join(VAULT_PATH, subPath) : VAULT_PATH;
...
const TOOLS = {
  ...
  'create-note': {
    ...
    async run({ filename, folder, content }) {
      ...
      return `Created: ${path.relative(VAULT_PATH, fp)}`;
    }
  },
  ...
  'edit-note': {
    ...
    async run({ filename, folder, operation, content, oldString, newString }) {
      ...
      return `Note ${operation}ed successfully\n\nModified file: ${path.relative(VAULT_PATH, fp)}`;
    }
  },
  ...
  'search-vault': {
    ...
    async run({ query, searchType = 'both', caseSensitive = false, fuzzy = false, path: subPath }) {
      return formatSearchResults(await search(query, searchType, caseSensitive, subPath, fuzzy), query);
    }
  },
  ...
  'list-notes': {
    ...
    async run({ folder, includeStats = false }) {
      const root = folder ? path.join(VAULT_PATH, folder) : VAULT_PATH;
      const files = await walkMd(root);
  ...
  'move-note': {
    ...
    async run({ filename, folder, newFilename, newFolder }) {
      ...
      return `Moved to ${path.relative(VAULT_PATH, dest)}`;
    }
  },
  ...
  'create-directory': {
    ...
    async run({ path: dirPath }) {
      await fs.mkdir(path.join(VAULT_PATH, dirPath), { recursive: true });
  ...
  'delete-note': {
    ...
    async run({ filename, folder }) {
      ...
      await fs.rename(src, path.join(TRASH_PATH, filename));
  ...
  'open-trash': {
    ...
    async run() {
      const entries = await fs.readdir(TRASH_PATH).catch(() => []);
  ...
  'recover-from-trash': {
    ...
    async run({ filename, targetFolder }) {
      const src = path.join(TRASH_PATH, filename);
      const dest = resolvePath(targetFolder, filename);
      await fs.mkdir(path.dirname(dest), { recursive: true });
      await fs.rename(src, dest);
      return `Recovered ${filename} from .trash → ${path.relative(VAULT_PATH, dest)}`;
    }
  }
};
...
// MCP streamable HTTP handler (GET for SSE, POST for JSON-RPC, DELETE for session)
app.use((req, res) => {
...
app.listen(PORT, '0.0.0.0', () => {
  console.error(`obsidian-mcp ${server._serverInfo.version} listening on port ${PORT}`);
});
